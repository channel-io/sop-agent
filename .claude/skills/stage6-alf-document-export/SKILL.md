...
2. Create `results/{company}/06_alf_documents/rules/` directory
...
- **2,000자 제한 (필수)**: 채널톡 ALF Rules API의 `instruction` 필드는 최대 2,000자. 모든 규칙 파일은 **제목 행을 제외하고 2,000자 이내**여야 한다.
  ...
  - 2,000자를 초과하는 파일은 **서브 섹션 단위로 추가 분할** (예: `08_special_rules.md` → `08a_*.md` + `08b_*.md`)
...
3. Write an expanded knowledge document for each item to `results/{company}/06_alf_documents/rag/`
...
2. **RAG 문서**: Compare item count in `rag_items.md` vs generated file count in `06_alf_documents/rag/`
...
[규칙 파일] results/{company}/06_alf_documents/rules/
  {파일 목록}

[RAG 지식 문서] results/{company}/06_alf_documents/rag/
  {파일 목록}

[다음 단계]
  1. 06_alf_documents/rules/ 파일 → ALF 시스템 프롬프트에 규칙별로 등록
  2. 06_alf_documents/rag/ 파일 → ALF RAG 지식 DB에 등록
