...
```bash
# In Codex — save each SOP as a separate file
...
```json
{
  ...
  "generated_by": "Codex Sonnet 4.5 (Stage 3 Pipeline)",
  "source_files": [
  ...
  "deployment": {
    "status": "ready",
    "target_platform": "Codex Skills / MCP Server",
...
- AI assistants (Codex Skills, MCP tools)
...
- You MUST generate SOP documents using **LLM (Codex)** directly
...
2. **LLM 직접 작성** - Python 스크립트 없이 Codex가 직접 작성
...
4. **Deploy**: Roll out via Codex Skills, MCP server, or training docs
